...
export function createNameUpdatedEvent(tokenId: BigInt, previousName: string, newName: string): NameUpdated {
  let nameUpdatedEvent = changetype<NameUpdated>(newMockEvent());

  nameUpdatedEvent.parameters = new Array();

  nameUpdatedEvent.parameters.push(new ethereum.EventParam('tokenId', ethereum.Value.fromUnsignedBigInt(tokenId)));
  nameUpdatedEvent.parameters.push(new ethereum.EventParam('previousName', ethereum.Value.fromString(previousName)));
  nameUpdatedEvent.parameters.push(new ethereum.EventParam('newName', ethereum.Value.fromString(newName)));
